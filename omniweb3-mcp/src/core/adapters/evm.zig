...
pub const EvmAdapter = struct {
    ...
    pub fn estimateGas(self: *EvmAdapter, call: EthCall) !u64 {
        const response = try self.provider.provider.estimateGas(call, .{});
        defer response.deinit();
        return response.response;
    }
    ...
    pub fn sendTransfer(
        ...
        const call = switch (tx_type) {
            .legacy => EthCall{ .legacy = .{ .from = from, .to = to, .value = amount } },
            else => EthCall{ .london = .{ .from = from, .to = to, .value = amount } },
        };

        const gas_estimate = try self.provider.provider.estimateGas(call, .{});
        defer gas_estimate.deinit();

        const envelope = switch (tx_type) {
            .legacy => blk: {
                const fee_estimate = try self.provider.provider.estimateFeesPerGas(call, null);
