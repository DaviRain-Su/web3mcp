//! Privy Sign and Send Transaction Tool
//!
...
/// Sign and send a transaction with a Privy wallet
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const sponsor_str = if (sponsor) "true" else "false";
    ...
    const body = if (std.mem.eql(u8, chain_type, "solana")) blk: {
        ...
        break :blk std.fmt.allocPrint(
            allocator,
            "{{\"method\":\"eth_sendTransaction\",\"caip2\":\"{s}\",\"params\":{{\"transaction\":\"{s}\"}},\"sponsor\":{s}}}",
            .{ caip2, transaction, sponsor_str },
        );
    } else {
