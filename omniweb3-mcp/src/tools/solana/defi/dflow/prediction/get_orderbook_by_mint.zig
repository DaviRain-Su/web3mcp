...
/// Get prediction market orderbook by mint address (dFlow).
///
/// Parameters:
...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const url = std.fmt.allocPrint(allocator, "{s}/{s}", .{ endpoints.dflow.pm_orderbook_by_mint, mint }) catch {
        return mcp.tools.ToolError.OutOfMemory;
    };
    defer allocator.free(url);

    const body = secure_http.dflowGet(allocator, url) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to fetch dFlow orderbook by mint: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
