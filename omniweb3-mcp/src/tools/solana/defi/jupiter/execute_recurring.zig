...
/// Execute a signed Jupiter recurring order transaction.
///
/// Parameters:
...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_base = mcp.tools.getString(args, "endpoint") orelse "https://api.jup.ag/recurring/v1/execute";
    ...
    const body = postHttp(allocator, endpoint_base, request_body, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to execute recurring: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
