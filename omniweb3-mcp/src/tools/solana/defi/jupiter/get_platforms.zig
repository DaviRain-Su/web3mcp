...
/// Get Jupiter portfolio platforms.
///
/// Parameters:
...
///
/// Returns JSON with available platforms
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.portfolio_platforms;
    ...
    const body = fetchHttp(allocator, endpoint_override, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to get platforms: {s}", .{@errorName(err)}) catch {
    ...
    const Response = struct {
        platforms: std.json.Value,
        endpoint: []const u8,
    };

    const response_value: Response = .{
        .platforms = parsed.value,
