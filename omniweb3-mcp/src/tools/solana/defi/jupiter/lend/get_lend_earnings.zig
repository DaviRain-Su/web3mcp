...
/// Get Jupiter Lend (Earn) earnings for a Solana account.
///
/// Parameters:
...
///
/// Returns JSON with lending earnings
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.lend_earnings;
    ...
    const body = fetchHttp(allocator, url, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to get lend earnings: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
    ...
    const Response = struct {
        account: []const u8,
        earnings: std.json.Value,
        endpoint: []const u8,
    };

    const response_value: Response = .{
        .account = account,
        .earnings = parsed.value,
