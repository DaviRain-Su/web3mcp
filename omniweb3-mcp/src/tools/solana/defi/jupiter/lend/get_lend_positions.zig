...
/// Get Jupiter Lend (Earn) positions for a Solana account.
///
/// Parameters:
...
///
/// Returns JSON with lending positions
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.lend_positions;
    ...
    const body = fetchHttp(allocator, url, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to get lend positions: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
