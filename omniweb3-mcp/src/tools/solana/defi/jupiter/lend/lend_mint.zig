...
/// Create a Jupiter Lend mint transaction.
/// Mints shares in exchange for deposited assets.
///
...
///
/// Returns JSON with mint transaction
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_base = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.lend_mint;
    ...
    const body = secure_http.securePost(allocator, endpoint_base, request_body, true, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to create lend mint: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
