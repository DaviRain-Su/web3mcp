...
/// Create a Jupiter Lend redeem transaction.
...
/// - mint: Base58 token mint to redeem (required)
/// - amount: Amount of shares to redeem (required)
/// - endpoint: Override Jupiter API endpoint (optional)
/// - insecure: Skip TLS verification (optional, default: false)
///
/// Returns JSON with redeem transaction
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_base = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.lend_redeem;
    ...
    const body = secure_http.securePost(allocator, endpoint_base, request_body, true, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to create lend redeem: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
