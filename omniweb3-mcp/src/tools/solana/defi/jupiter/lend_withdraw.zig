...
///
/// Returns JSON with withdraw transaction
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_base = mcp.tools.getString(args, "endpoint") orelse "https://api.jup.ag/lend/v1/earn/withdraw";
    ...
    const body = postHttp(allocator, endpoint_base, request_body, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to create lend withdraw: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
