...
/// Get Jupiter recurring (DCA) orders for a Solana account.
///
/// Parameters:
...
///
/// Returns JSON with recurring orders
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.recurring_orders;
    ...
    const body = fetchHttp(allocator, url, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to get recurring orders: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
