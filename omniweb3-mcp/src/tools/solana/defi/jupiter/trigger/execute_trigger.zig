...
/// Execute a signed Jupiter trigger order transaction.
///
...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_base = mcp.tools.getString(args, "endpoint") orelse "https://api.jup.ag/trigger/v1/execute";
    ...
    const body = secure_http.securePost(allocator, endpoint_base, request_body, true, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to execute trigger: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
