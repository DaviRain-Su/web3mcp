...
/// Get available routers in Jupiter Ultra routing engine.
///
/// Parameters:
...
///
/// Returns JSON with available routers
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.ultra_routers;
    ...
    const body = secure_http.secureGet(allocator, endpoint_override, use_api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to get routers: {s}", .{@errorName(err)}) catch {
    ...
    const Response = struct {
        routers: std.json.Value,
        endpoint: []const u8,
    };

    const response_value: Response = .{
        .routers = parsed.value,
