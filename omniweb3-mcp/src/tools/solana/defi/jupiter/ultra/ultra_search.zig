...
/// Search tokens via Jupiter Ultra API.
///
/// Parameters:
...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const endpoint_override = mcp.tools.getString(args, "endpoint") orelse endpoints.jupiter.ultra_search;
    ...
    const body = secure_http.secureGet(allocator, url, use_api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to search: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
