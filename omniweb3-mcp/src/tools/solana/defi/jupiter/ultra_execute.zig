...
/// - request_id: Request ID from the order response (optional)
...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const request_id = mcp.tools.getString(args, "request_id");
    ...
    if (request_id) |rid| {
        request_obj.put("requestId", .{ .string = rid }) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
    }
    ...
    const body = postHttp(allocator, endpoint_override, request_body, api_key, insecure) catch |err| {
        const msg = std.fmt.allocPrint(allocator, "Failed to execute Ultra order: {s}", .{@errorName(err)}) catch {
            return mcp.tools.ToolError.OutOfMemory;
        };
