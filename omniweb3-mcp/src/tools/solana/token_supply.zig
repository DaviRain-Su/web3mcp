...
pub fn handle(allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    var result = adapter.client.json_rpc.callWithResult(allocator, "getTokenSupply", .{ .array = params_arr }) catch |err| {
    ...
    const value = result.value orelse {
        return mcp.tools.errorResult(allocator, "Missing token supply result") catch {
            return mcp.tools.ToolError.InvalidArguments;
        };
    };
    ...
    const response = std.fmt.allocPrint(
        allocator,
        "{{\"chain\":\"solana\",\"mint\":\"{s}\",\"network\":\"{s}\",\"endpoint\":\"{s}\",\"supply\":{s}}}",
        .{ mint_str, network, adapter.endpoint, supply_json },
    ) catch {
        return mcp.tools.ToolError.OutOfMemory;
    };
