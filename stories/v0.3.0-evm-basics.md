...
- [x] 初始化 EVM runtime (std.Io.Threaded)
- [x] 验证 Zig 0.16 兼容性

### EVM RPC Client (via zabi)
- [x] HTTP Provider 连接
- [x] 支持 getAddressBalance（native token）
- [x] 支持 sendRawTransaction
- [x] 支持 waitForTransactionReceipt
- [x] 支持 getGasPrice / estimateGas
- [x] 可配置 RPC endpoint（多链支持）

### Wallet/Signing (via zabi)
- [x] 从私钥创建 Wallet
- [x] 从文件加载私钥（类似 Solana 模式）
- [x] 支持 EIP-1559 交易签名
- [x] 支持 Legacy 交易签名

### Tools Implementation
- [x] `tools/evm_balance.zig` 实现 EVM 余额查询
  - 参数: chain, address, network
  - 返回: balance (wei), balance (ETH/native)
- [x] `tools/evm_transfer.zig` 实现原生代币转账
  - 参数: to_address, amount, network, chain, keypair_path (optional)
  - 返回: tx_hash, tx details
- [x] `tools/registry.zig` 注册新工具

### Multi-Chain Configuration
- [x] Ethereum: mainnet, sepolia, goerli
- [x] Avalanche C-Chain: mainnet, fuji
- [x] BNB Chain: mainnet, testnet
- [x] 支持自定义 RPC endpoint

### Configuration
- [x] 支持 EVM_PRIVATE_KEY 环境变量
- [x] 支持 ~/.config/evm/keyfile.json 配置文件
